...
bathy_inlat=str(sys.argv[5]) # lat field name in the input/output file
bathy_inlon=str(sys.argv[6]) # lon field name in the input/output file
order=int(sys.argv[7]) # Order of the Shapiro filter
napp=int(sys.argv[8]) # Number of Shapiro filter applications
scheme=int(sys.argv[9]) # type of boundary scheme to use ( only option 1 implemented = No change at wall, constant order )  
...
def shapiro1D(Finp,order,scheme):
     ...
     ###########
     fourk=[2.500000E-1,6.250000E-2,1.562500E-2,3.906250E-3,9.765625E-4,2.44140625E-4,6.103515625E-5,1.5258789063E-5,3.814697E-6,9.536743E-7,2.384186E-7,5.960464E-8,1.490116E-8,3.725290E-9,9.313226E-10,2.328306E-10,5.820766E-11,1.455192E-11,3.637979E-12,9.094947E-13] # Cos'e' D???
...
# Create the new fields to be appended in the input/output netCDF
print ('I am going to add the new bathymetry field: ',bathy_outname)
diff_bathy=bathy_infield.createVariable('diff',np.float64,(bathy_inlat,bathy_inlon))
diff_bathy.units = 'm'

print ('I am writing the diff field in the input/output netCDF..')
...
# cp /data/oda/ec04916/PHYSW_DATA/BATHY/GEBCO_2014_2D_-19.0_30.0_37.0_46.0.nc /work/oda/ag15419/tmp/twd_shapiro/
# python shapiro4twd.py /work/oda/ag15419/tmp/twd_shapiro/ GEBCO_2014_2D_-19.0_30.0_37.0_46.0.nc elevation smoothed_elevation lat lon 2 2 1
